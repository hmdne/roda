...
class Roda
  module RodaPlugins
    ...
    #            templates, defaults to 'erb'. It can also be an array, in which case all engines
    #            will be tried and the first will be used.
    ...
    #           application's :root option (the process's working directory by default). It can also
    #           be an array in which case all paths will be tried and the first will be used.
    #
    ...
    module Render
      ...
      module InstanceMethods
        ...
        def template_path_and_engine(opts)
          path, engine = nil, nil
          views_and_engines = Array(opts[:views]).product(Array(opts[:engine]))
          views_and_engines.each do |views, eng|
            path = "#{views}/#{template_name(opts)}.#{eng}"
            engine = eng
            break if views_and_engines.length == 1 || File.exist?(path)
          end
